...
for DIR in "${DIRS[@]}"; do
    ...
    for DOCKERFILE in "$DIR"/Dockerfile.*; do
        ...
        docker buildx build --load --cache-from=type=local,src=/tmp/.buildx-cache --cache-to=type=local,dest=/tmp/.buildx-cache --build-arg BASE_DIR="${DIR}" --progress plain --platform=linux/amd64 -t "$IMAGE_NAME" -f "$DOCKERFILE" .
